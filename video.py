...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--use-email", type=bool, default=0,help="boolean indicating if Emails need to be send or not")
        ...
        if detection is not None:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls_pred in detection:
                ...
                if (int(cls_pred) == 0):
                    ...
                    cv2.imwrite("output/detected.jpg", org_frame)

                    #SEND EMAIL NOTIFICATION
                    mail_content = 'Dear Admin,\n Some person without facemask has been detected. Below is the attached image of the person.'
                    #The mail addresses and password
                    sender_address = 'facemaskdetectionpro@gmail.com'
                    sender_pass = 'kugymtkicqjrrlqm'
                    receiver_address = 'mimiyoung2004@gmail.com'
                    #Setup the MIME
                    message = MIMEMultipart()
                    message['From'] = sender_address
                    message['To'] = receiver_address
                    message['Subject'] = 'Facemask Detection using Yolov3 with Email Alert.'   #The subject line
                    #The body
                    message.attach(MIMEText(mail_content, 'plain'))

                    filename = "output/detected.jpg"

                    with open(filename, "rb") as f:
                        file_data =  f.read()
                    image = MIMEImage(file_data, name=os.path.basename(filename))
